...
public partial class ThirdPersonPlayerInputsSystem : SystemBase
{
    protected override void OnCreate()
    {
        ...
        RequireForUpdate(QueryBuilder().WithAll<ThirdPersonPlayer, ThirdPersonPlayerInputs>().Build());
    }
    ...
    protected override void OnUpdate()
    {
        ...
        foreach (var (playerInputs, player) in Query<RefRW<ThirdPersonPlayerInputs>, ThirdPersonPlayer>())
        {
...
public partial struct ThirdPersonPlayerVariableStepControlSystem : ISystem
{
    ...
    {
        state.RequireForUpdate(QueryBuilder().WithAll<ThirdPersonPlayer, ThirdPersonPlayerInputs>().Build());
    }
    ...
    {
        foreach ((ThirdPersonPlayer player, ThirdPersonPlayerInputs playerInputs) in Query<ThirdPersonPlayer, ThirdPersonPlayerInputs>().WithAll<Simulate>())
        {
            if (HasComponent<OrbitCameraControl>(player.ControlledCamera))
            {
                OrbitCameraControl cameraControl = GetComponent<OrbitCameraControl>(player.ControlledCamera);
                
                cameraControl.FollowedCharacterEntity = player.ControlledCharacter;
                cameraControl.Look = playerInputs.CameraLookInput;
                cameraControl.Zoom = playerInputs.CameraZoomInput;

                SetComponent(player.ControlledCamera, cameraControl);
            }
        }
    }
}
...
public partial struct ThirdPersonPlayerFixedStepControlSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        uint fixedTick = GetSingleton<FixedTickSystem.Singleton>().Tick;

        foreach (var (playerInputs, player) in Query<RefRW<ThirdPersonPlayerInputs>, ThirdPersonPlayer>().WithAll<Simulate>())
        {
            if (HasComponent<ThirdPersonCharacterControl>(player.ControlledCharacter))
            {
                ThirdPersonCharacterControl characterControl = GetComponent<ThirdPersonCharacterControl>(player.ControlledCharacter);

                float3 characterUp = MathUtilities.GetUpFromRotation(GetComponent<LocalTransform>(player.ControlledCharacter).Rotation);
                
                // Get camera rotation data, since our movement is relative to it
                quaternion cameraRotation = quaternion.identity;
                if (HasComponent<LocalTransform>(player.ControlledCamera))
                {
                    cameraRotation = GetComponent<LocalTransform>(player.ControlledCamera).Rotation;
                }
                ...
                SetComponent(player.ControlledCharacter, characterControl);
            }
        }
    }
}